...
# ==========================================
    ...
    with col1:
        age = st.number_input("Driver Age", min_value=18, max_value=90, value=35)
        income = st.number_input("Annual Income ($)", min_value=0.0, value=55000.0, step=1000.0)
        safety = st.slider("Safety Rating", min_value=0, max_value=100, value=75)
        gender = st.selectbox("Gender", ["MALE", "FEMALE"])
        marital = st.selectbox("Marital Status", ["MARRIED", "SINGLE", "DIVORCED"])
        education = st.selectbox("High Education Completed", ["College / Degree (Yes)", "High School (No)"])

    with col2:
        claim_amt = st.number_input("Total Claim Amount ($)", min_value=500.0, value=25000.0, step=1000.0)
        injury_amt = st.number_input("Injury Claim Portion ($)", min_value=0.0, value=4000.0, step=500.0)
        deductible = st.selectbox("Policy Deductible ($)", [500, 1000, 1500, 2000])
        premium = st.number_input("Annual Premium ($)", min_value=100.0, value=1350.0, step=50.0)
        past_claims = st.number_input("Past Claims Count", min_value=0, max_value=10, value=0)
        liab_prct = st.slider("Liability %", min_value=0, max_value=100, value=25)

    with col3:
        accident_site = st.selectbox("Accident Site", ["Highway", "Intersection", "Local", "Parking Lot"])
        witness = st.selectbox("Witness Present", ["YES", "NO"])
        police_rep = st.selectbox("Police Report Filed", ["YES", "NO"])
        days_open = st.number_input("Days Claim Open", min_value=1, max_value=60, value=7)
        form_defects = st.number_input("Form Defect Flags", min_value=0, max_value=10, value=0)
        veh_cat = st.selectbox("Vehicle Category", ["Medium", "Large", "Small / Other"])
        veh_color = st.selectbox("Vehicle Color", ["silver", "blue", "gray", "red", "white", "other"])
    ...
    if st.button("🚀 Evaluate Fraud Probability", type="primary", use_container_width=True):
        if best_model is not None and feature_cols is not None:
            ...
            row_dict['age_of_driver'] = age
            row_dict['safety_rating'] = safety
            row_dict['annual_income'] = income
            row_dict['high_education'] = 1 if "Yes" in education else 0
            row_dict['past_num_of_claims'] = past_claims
            row_dict['liab_prct'] = liab_prct
            row_dict['police_report'] = 1 if police_rep == "YES" else 0
            row_dict['total_claim'] = claim_amt
            row_dict['injury_claim'] = injury_amt
            row_dict['policy_deductible'] = deductible
            row_dict['annual_premium'] = premium
            row_dict['days_open'] = days_open
            row_dict['form_defects'] = form_defects
            row_dict['gender_M'] = 1 if gender == "MALE" else 0
            row_dict['marital_status_1'] = 1 if marital == "MARRIED" else 0
            row_dict['witness_present_1'] = 1 if witness == "YES" else 0
            
            # Category & accident site
            if f'accident_site_{accident_site}' in row_dict:
                row_dict[f'accident_site_{accident_site}'] = 1
            if f'vehicle_category_{veh_cat}' in row_dict:
                row_dict[f'vehicle_category_{veh_cat}'] = 1
            if f'vehicle_color_{veh_color.lower()}' in row_dict:
                row_dict[f'vehicle_color_{veh_color.lower()}'] = 1
